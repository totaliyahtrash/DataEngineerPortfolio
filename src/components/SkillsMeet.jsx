...
export default function SkillsMeet() {
  ...
  const titleInView = useInView(sectionRef, { once: true, margin: "-5% 0px -5% 0px" });
  const gridInView = useInView(gridRef, { once: true, margin: "-5% 0px -5% 0px" });
  const controlsInView = useInView(controlsRef, { once: true, margin: "-5% 0px -5% 0px" });
            ...
            <>
              {skillsList.map((skill, idx) => (
                <motion.div
                  ...
                  transition={{ type: "spring", stiffness: 240, damping: 20, delay: idx * 0.06 }}
